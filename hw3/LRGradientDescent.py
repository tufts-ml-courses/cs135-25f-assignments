...
class LogisticRegressionGradientDescent():
    ...
    * the extra L2 penalty term will only ever add to the loss
    ...
    def predict_proba(self, x_NF):
        ...
        s_N = np.zeros(N)  # TODO compute weights times x plus b
        proba1_N = 1.0 * s_N  # TODO use 'logistic_sigmoid' to make probas
        ...
        for iter_id in range(self.num_iterations + 1):
            ...
            if self.verbose:
                if iter_id < 20 or (iter_id % 20 == 0) or (iter_id % 20 == 1):
                    ...
                    lineB = ' avg_abs_grad % 10.6f  w[0] % 7.3f b % 7.3f' % (
    ...
    def calc_loss(self, wtil_G, xone_NG, y_N):
        ...
        L2_loss = 0.5 * np.sum(np.square(wtil_G[:-1]))
        
        # Second term: Calc log loss by summing over examples
        s_N = np.matmul(xone_NG, wtil_G)
        # Convert binary y values so 0 becomes +1 and 1 becomes -1
        minus1_tothe_y = (-1) ** np.round(y_N)
        # Use numerically stable logsumexp calculation of log loss
        ...
        if self.use_base2_for_BCE:
            scoreBCE_loss = scoreBCE_loss / np.log(2)

        # Add two terms together 
        # return the sum rescaled so it is tight upper bound of error rate
        return (L2_loss + self.C * scoreBCE_loss) / (N * self.C)
